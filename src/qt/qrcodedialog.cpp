...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("SOC")
    updateDisplayUnit();
}
...
{
    QString ret = QString("socialcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non SOC unit input in lnReqAmount, we generate the URI with SOC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::SOC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
